...
Before SSH-ing into the Pi, set up static IPs in your router. This prevents the Pi or camera IP from changing on reboot, which would break the RTSP stream URL and any scripts referencing the Pi's address.
...
4. Note both IPs — you'll need them throughout this guide
...
- **Camera services** — note which are running. We'll disable them all in Section 5
...
## Section 5: Disable Default Camera Services *(Required if using a camera)*
...
**Skip Sections 5–6 if you don't have a camera.** OctoPrint works fine without one — you just won't have Obico AI failure detection or a live camera feed.
...
## Section 6: Install go2rtc + Camera Pipeline *(Required if using a camera)*
...
## Section 11: Install and Link Obico *(Recommended — requires camera from Section 6)*
...
- **Resolution:** Medium (High caused 45-60 second delays in testing due to encoding overhead on the Pi 4)
- **Target FPS:** 5 (matches go2rtc's 5fps output — higher settings just waste CPU re-encoding)

**📱 In the Obico app** (app.obico.io or mobile app):
- **Action on failure:** Pause print + notify

> **Why not High resolution?** High quality causes severe buffering (45-60 second delays) on the Pi 4 because Obico's internal ffmpeg re-encodes your MJPEG stream to H.264 for cloud upload. At High, this encoding overwhelms the Pi's CPU. Medium at 5fps is the sweet spot — good enough for AI detection, low enough CPU load to leave headroom for OctoPrint's serial communication.

### 11.4: Verify Snapshot Speed Under Obico

After linking, verify that Obico's own ffmpeg process hasn't degraded snapshot performance.
...
**✅ Expected:** HTTP 200, ~125KB, <0.3 seconds. This is what Obico's AI analyzes — full 1920×1080 JPEG snapshots from the Buddy camera.

> **Understanding the ffmpeg processes:** After Obico is linked, you'll see **two ffmpeg processes** in `ps aux`. The first (spawned by go2rtc) transcodes RTSP to MJPEG at ~47% CPU. The second (spawned by Obico) re-encodes MJPEG to H.264 at ~13% CPU for cloud streaming at 854×480. Both are expected and the combined load (~60% single-core) is sustainable on a Pi 4.
...
**⌨️ After reconnecting (~60 seconds), paste:**
```bash
...
**✅ Expected:**
- All active services: `active`
- All disabled services: `masked`
...
**⌨️ Snapshot speed test** — should be <0.3s. If slow, keepalive may have died:
```bash
