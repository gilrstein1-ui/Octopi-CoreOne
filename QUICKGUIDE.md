...
## 3. Disable Default Camera Services

*OctoPi ships with webcamd and other streaming services. We replace them with go2rtc. Leaving them causes port conflicts.*
...
## 4. Install go2rtc + Camera Pipeline

*Bridges your Buddy camera's RTSP stream to MJPEG for OctoPrint + JPEG snapshots for Obico AI. The keepalive service keeps ffmpeg warm so snapshots return in ~0.2s instead of ~5s. Skip if no camera.*
...
*AI failure detection — watches your camera and auto-pauses on spaghetti/blob-of-death.*
...
|---|---|---|
| **Service status** | After reboot, or if something seems off | `for svc in octoprint go2rtc go2rtc-keepalive zram-swap haproxy; do printf "%-22s %s\n" "$svc:" "$(systemctl is-active $svc)"; done` |
| **Snapshot speed** | If Obico seems slow or camera feed is stale | `curl -s -o /dev/null -w "%{time_total}s\n" "http://127.0.0.1:1984/api/frame.jpeg?src=buddy_mjpeg"` |
